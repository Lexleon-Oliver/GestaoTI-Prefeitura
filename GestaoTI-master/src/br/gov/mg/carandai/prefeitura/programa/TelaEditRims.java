...
public class TelaEditRims extends javax.swing.JInternalFrame {


    
    
    public TelaEditRims(int id) throws ClassNotFoundException {
        initComponents();
        Rims rim =new Rims();
        RIMSDAO rdao = new RIMSDAO();
        rim = rdao.listarPorID(id);
        this.preeencherCampos(rim);
    }
    
    public void preeencherCampos(Rims rim) throws ClassNotFoundException{
        
        CampoData.setText(rim.getDataPed());
        CampoFicha.setText(rim.getFicha());
        CampoFonte.setText(rim.getFonte());
        CampoForn.setText(rim.getFornec());
        CampoNumProc.setText(rim.getNumProc());
        CampoNumProc2.setSelectedItem(rim.getAnoProc());
        CampoNumRims.setText(rim.getNumRims());
        CampoNumRims2.setSelectedItem(rim.getAnoRims());
        CampoMat.setText(rim.getProd());
        CampoSol.setText(rim.getSolicit());
        CampoSim.setSelected(rim.isEntregue());
        CampoNao.setSelected(!rim.isEntregue());
        CampoID.setText(String.valueOf(rim.getId()));
    }
    ...
    private void initComponents() {
        ...
        CampoID.setFormatterFactory(new javax.swing.text.DefaultFormatterFactory(new javax.swing.text.NumberFormatter(new java.text.DecimalFormat("#0"))));
    ...
    private void BotaoSalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BotaoSalvarActionPerformed
        ...
        if("".equals(CampoNumRims.getText())) {
            ...
            if(JOptionPane.showConfirmDialog(null, "Deseja alterar o registro?","Confirmar Alteração!",JOptionPane.YES_NO_OPTION)== 0){
                Rims  rims= new Rims();
                RIMSDAO rdao = new RIMSDAO();
                ...
                rims.setId(Integer.valueOf(CampoID.getText()));
                
                try {
                    rdao.atualizar(rims);
                } catch (ClassNotFoundException ex) {
                    Logger.getLogger(TelaEditRims.class.getName()).log(Level.SEVERE, null, ex);
                }
                limparCampos();
            }else{
                CampoNumRims.requestFocus();
