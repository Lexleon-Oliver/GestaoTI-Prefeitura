...
public class TelaEditRoteador extends javax.swing.JInternalFrame {

    
    
    
    public TelaEditRoteador(String mac) throws ClassNotFoundException {
        initComponents();
        Roteador router =new Roteador();
        router = RoteadorDAO.listarPorMAC(mac);
        this.preeencherCampos(router);
    }
    
    public void preeencherCampos(Roteador roteador) throws ClassNotFoundException{
        
        CampoRedeWifi.setText(roteador.getRedewifi());
        CampoMarca.setText(roteador.getMarca());
        CampoModelo.setText(roteador.getModelo());
        CampoSenhaWifi.setText(roteador.getSenhawifi());
        CampoAndar.setSelectedIndex(Integer.parseInt(roteador.getAndar()));
        CampoMAC.setText(roteador.getMac());
        CampoDepartamento.setText(roteador.getDepartamento());
        CampoLogin.setText(roteador.getLogin());
        CampoSenha.setText(roteador.getSenha());
        CampoIP.setText(roteador.getIp());
        CampoPrefSim.setSelected(roteador.isPrefeitura());
        CampoPrefNao.setSelected(!roteador.isPrefeitura());
    }
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Alteração de Roteador", javax.swing.border.TitledBorder.CENTER, javax.swing.border.TitledBorder.TOP, new java.awt.Font("Tahoma", 1, 16))); // NOI18N
      ...
      if("".equals(CampoMarca.getText())) {
           ...
           CampoSenhaWifi.requestFocus();
            ...
            try {
                ...
                dao.atualizarRoteador(router);
                limparCampos();
                dispose();

            } catch (ClassNotFoundException ex) {
                Logger.getLogger(TelaEditRoteador.class.getName()).log(Level.SEVERE, null, ex);
            }
            }else{
                CampoMarca.requestFocus();
            }
